	...
	onMount(() => {
		...
		(window as any).onYouTubeIframeAPIReady = () => {
			birthdayVideos.forEach((video, index) => {
				...
				players[video.name] = new (window as any).YT.Player(
...
<main>
	...
	<div class="card-container">
		<button class="nav-button prev" on:click={previousCard} disabled={currentCardIndex === 0}
			>❮</button
		>
		<div class="card-slider" style="transform: translateX(-{currentCardIndex * 100}%)">
			{#each romanticMessages as message (message)}
				<div class="card message-card">
					<p class="message">{message}</p>
				</div>
			{/each}
			{#each birthdayVideos as video, index (video.name)}
		...
		<button
			...
			on:click={nextCard}
			disabled={currentCardIndex === romanticMessages.length + birthdayVideos.length - 1}>❯</button
		>
	</div>
	...
	<div class="love-note">
		...
		<p>
			Amadea, my love, words cannot express how much you mean to me. Your presence in my life is a
			...
			love, laughter, and unforgettable memories together.
		</p>
